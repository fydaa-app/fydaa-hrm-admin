...
export default function EditAdvisor({ isOpen, onClose, advisor }: EditAdvisorProps) {
  ...
  const validateForm = () => {
    ...
    // Photo is required - either existing or new
    if (!photo && !advisor.photo) {
  ...
  const handleUpdateAdvisor = async (e: React.FormEvent) => {
    ...
    try {
      const response = await advisorServiceApi.updateAdvisor(advisorMetadata.id, {
        ...advisorMetadata,
        photo: photo || advisor.photo, // Use new photo or keep existing
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[99999]">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto dark:bg-gray-800">
        ...
        <form onSubmit={handleUpdateAdvisor} className="p-4 space-y-4">
          ...
          <div>
            <Label htmlFor="photo">
              Photo * {advisor.photo && '(Upload new to replace)'}
            </Label>
            <input
              id="photo"
              type="file"
            ...
            />
            {!photo && !advisor.photo && errors.photo && (
              <p className="text-red-500 text-sm mt-1">{errors.photo}</p>
            )}
            {photo && (
              <div className="flex items-center justify-between mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  📷 New: {photo.name} ({(photo.size / 1024).toFixed(2)} KB)
                </p>
                <button
                  type="button"
          ...
          <div>
            <Label htmlFor="attachment1">
              Attachment 1 {advisor.attachment1 && '(Upload new to replace)'}
            </Label>
            ...
            {attachment1 && (
              <div className="flex items-center justify-between mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  📎 New: {attachment1.name} ({(attachment1.size / 1024).toFixed(2)} KB)
                </p>
                <button
                  type="button"
          ...
          <div>
            <Label htmlFor="attachment2">
              Attachment 2 {advisor.attachment2 && '(Upload new to replace)'}
            </Label>
            ...
            {attachment2 && (
              <div className="flex items-center justify-between mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  📎 New: {attachment2.name} ({(attachment2.size / 1024).toFixed(2)} KB)
                </p>
                <button
                  type="button"
